// Cover page with black background and white text
#set page(
  paper: "us-letter",
  fill: rgb(0, 0, 0), // Black background
  margin: 1in,
)
#set text(
...
#align(center)[
  // #v(5%) // Vertical space to center content
  #text(size: 70pt, weight: "bold")[PURE BASH BIBLE]
  #image("shell.svg", height: 300pt)
  // #v(0%) // Vertical space to center content
  #text(size: 50pt, weight: "bold")[DYLAN ARAPS]
]

// Reset page settings for subsequent pages
#pagebreak()


#set page(
  fill: white, // Black background
  paper: "us-letter",
  numbering: "1",
  columns: 1,
)
...
#show raw.where(block: true): it => block(
  ...
  fill: luma(243),
  radius: 5pt,
  pad(left: 1em, right: 1em, top: 1em, bottom: 1em, it),
)
...
#table(
  ...
  [`${VAR:OFFSET:LENGTH}`], [Get substring from `N` character to `N` character. #linebreak() (`${VAR:10:10}`: Get sub-string from char `10` to char `20`)],
